...
impl<'a> SymbolValues<'a> {
    ...
    /// Mark a symbol as falsy-in-boolean-context. Set by `init_symbol_value`
    /// after `init_value`, for a write-once falsy binding whose value-context
    /// constant was withheld (see [`SymbolValue::boolean_falsy`]).
    pub fn set_boolean_falsy(&mut self, symbol_id: SymbolId) {
        if let Some(symbol_value) = self.values[symbol_id].as_mut() {
            symbol_value.boolean_falsy = true;
        }
    }
}
