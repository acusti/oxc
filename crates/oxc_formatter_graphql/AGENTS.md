...
- Parses with a fork of apollo-parser , pinned via `rev` in the workspace `Cargo.toml`
  - Base: 0.8.6 (October 2021 spec)
  ...
  - Both default to off;
    - `format.rs` enables them explicitly via `allow_executable_descriptions` / `allow_legacy_fragment_variables` on `Parser`
...
Adding any of these to the fork needs new grammar in apollo-parser (unlike `@oneOf`, these are genuinely new syntax).
